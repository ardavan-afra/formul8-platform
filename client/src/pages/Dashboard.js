...
const Dashboard = () => {
  ...
  const { data: projectsData, isLoading: projectsLoading } = useQuery(
    'recent-projects',
    () => api.get('/projects?limit=6').then(res => res.data),
    { staleTime: 5 * 60 * 1000 }
  );
      ...
      return [
        {
          ...
          value: recentProjects.filter(p => p.professor._id === user.id && p.status === 'active').length,
          icon: BookOpen,
          color: 'text-blue-600'
        },
        {
        ...
        {recentProjects.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {recentProjects.slice(0, 6).map((project) => (
              <div key={project._id} className="border border-gray-200 rounded-lg p-4 hover:shadow-sm transition-shadow duration-200">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {applications.slice(0, 3).map((application) => (
              <div key={application._id} className="flex justify-between items-center p-3 border border-gray-200 rounded-lg">
                <div>
